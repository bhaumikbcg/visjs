function createEdgeData(duplicateData, edgeData){
    ...
    for(var i = 0; i < duplicateData.length-1; i++){
        ...
        else if((duplicateData[i].level > duplicateData[i+1].level && duplicateData[i+1].id !== 0) || (duplicateData[i].level === duplicateData[i+1].level)){
            var previousConnection = getCurrent(duplicateData[i+1].level);
